...
async function makePdf(b:any){
  const pdf=await PDFDocument.create();let page=pdf.addPage([612,792]);const font=await pdf.embedFont(StandardFonts.Helvetica),bold=await pdf.embedFont(StandardFonts.HelveticaBold);let y=742;
  ...
  for(const l of (Array.isArray(b.lines)?b.lines:[])){if(y<150){page=pdf.addPage([612,792]);y=742;}const desc=String(l.description||"").slice(0,55);text(desc,56,y,9);text(String(Number(l.qty||0)),394,y,9);text(usd(l.rate),442,y,9);text(usd(Number(l.qty||0)*Number(l.rate||0)),505,y,9);y-=22;}
...
serve(async(req)=>{
  ...
  try{
    if(!RESEND_API_KEY)return json({ok:false,error:"Invoice email service is not configured yet."},503);
    const auth=req.headers.get("authorization")||"";if(!auth.startsWith("Bearer "))return json({ok:false,error:"Unauthorized"},401);
    const b=await req.json();if(!b.to||!b.number||!b.company)return json({ok:false,error:"Missing required invoice fields."},400);
    ...
    const r=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${RESEND_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify(payload)});const data=await r.json();if(!r.ok)return json({ok:false,error:data?.message||"Email failed to send."},502);return json({ok:true,id:data.id},200);
