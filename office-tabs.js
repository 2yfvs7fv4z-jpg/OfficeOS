...
function relocate(){
 ...
 const data=officeCards().find(c=>heading(c)==='Data');if(data&&settingsHost&&data.parentElement!==settingsHost){data.dataset.relocated='settings';data.classList.add('relocated-card');settingsHost.appendChild(data)}
...
window.officeJumpTo=function(key){jump(officeTarget(key),document.getElementById('officeJumpNav'),key)};
window.settingsJumpTo=function(key){jump(settingsTarget(key),document.getElementById('settingsJumpNav'),key)};
